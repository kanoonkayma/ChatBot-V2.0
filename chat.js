...
export async function executaChat(mensagem) {
  //permitir ter dados do tamanho do histórico
  console.log("Tamanho do histórico: " + (await chat.getHistory()).length);
  
  //comparar mensagem com o documento pra compatibilidade
  let doc = await incorporarPergunta(mensagem, documentos);

  let prompt = mensagem + "talvez esse texto te ajude a formular a resposta " + doc.text;
